...
must_haves:
  truths:
    ...
    - "User can log out from any page in the application (via shared Layout header)"
  artifacts:
    ...
    - path: "frontend/src/components/Layout.tsx"
      provides: "Shared layout with header and logout button for all protected pages"
      min_lines: 30
  key_links:
...
Create frontend/src/components/Layout.tsx (shared layout with logout on every page):
```typescript
...
Create frontend/src/pages/Dashboard.tsx (uses Layout for consistent header/logout):
```typescript
import { Link } from 'react-router-dom';
...
Create frontend/src/pages/ProjectForm.tsx (uses Layout for consistent header/logout):
```typescript
